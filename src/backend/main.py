...
async def lifespan(app: FastAPI):
    ...
    db_dir=os.path.dirname(settings.DATABASE_URL.split("///")[-1]); model_dir=os.path.dirname(settings.MODEL_PATH); scaler_dir=os.path.dirname(settings.SCALER_PATH); ae_model_dir = os.path.dirname(settings.AE_MODEL_PATH); ae_thresh_dir = os.path.dirname(settings.AE_THRESHOLD_PATH)
...
origins = ["http://localhost", "http://localhost:8080", "http://127.0.0.1:8050", "null"]; app.add_middleware(CORSMiddleware, allow_origins=origins, allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
app.include_router(prediction.router)
app.include_router(logging_router.router)
app.include_router(attack_types_router.router) # Include attack types router
@app.get("/", tags=["Root"])
async def read_root(): return {"message": f"{settings.PROJECT_NAME} is running!"}
if __name__ == "__main__": logging.info("Starting Uvicorn server for WIDS API..."); uvicorn.run( "main:app", host="0.0.0.0", port=8000, reload=True, log_level="info")
